...
def get_history():
	...
	park_id = request.args(0) or 'index'
	interval = request.vars.interval or 'index'
	if not(park_id and park_id.isdigit()): raise HTTP(404)
	data = server.DataManager.getFreeSlotsByTimeAndParkingArea(int(park_id), 24*60*7)
	output = []
	for element in data:
	...
	return response.render('generic.json', {'park_%s' % park_id:{'data':output}})	
